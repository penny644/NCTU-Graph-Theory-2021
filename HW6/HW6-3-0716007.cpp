...
public:
    ...
    void heapify(long long int dst, long long int length){
	    long long int left,right,min,tmp;
	    left=2*dst;
		right=2*dst+1;
    ...
    void decrease(long long int dst, long long int weight){
		long long int index_tmp,tmp;
		index_tmp = index[dst];
    ...
    void insert(long long int dst, long long int weight){
	    adjnode tmp;
	    tmp.dst = dst;
	    tmp.weight = weight;
    ...
    long long int extract(){
	    long long int min = adjnode_list[1].dst;
...
void newedge(vector<adjnode> graph[] ,long long int s, long long int dst, long long int weight) {
    adjnode tmp;
    tmp.dst = dst-1;
    tmp.weight = weight;
...
int main(){
	long long int n,m,inputedge[3],*ans,*c,s_tmp,color;
	long long int i,j,k;
	...
	for(i=0;i<m;i++){
		...
		if(c[inputedge[0]-1]!=c[inputedge[1]-1]){
			if(inputedge[2]<ans[inputedge[0]-1]) ans[inputedge[0]-1]=inputedge[2];
			if(inputedge[2]<ans[inputedge[1]-1]) ans[inputedge[1]-1]=inputedge[2];
		}
		else{
